...
## QA spot-check – Regression audit (2025-12-19)
- Confirmed the primary note regression continues to pass all 89 fixtures via `python temp/run_tests.py --tests temp/tests.json --style temp/texas-greenbook-15th-edition.csl --expected temp/expected.txt`; the pre-patch run captured the lingering `related` warning for baseline reference.【b2c427†L1-L20】
- Exercised the focused parenthetical suites in both note and bibliography modes to verify slip-opinion URL placement and petition-history strings remain stable (`python temp/run_tests.py --tests temp/tests_parentheticals.json --style temp/texas-greenbook-15th-edition.csl --expected temp/expected_parentheticals_notes.txt` and the bibliography variant with `--mode bibliography`), documenting the same warning state before suppression.【951a13†L1-L8】【d48d2e†L1-L6】
- Ran the grouped-leaders TOA harness (`python temp/run_tests.py --tests temp/tests_toa.json --style temp/texas-greenbook-15th-toa-grouped-leaders.csl --expected temp/expected_toa_grouped_leaders.txt`) to confirm dotted leader alignment and preserved the warning in the captured log for comparison.【58c934†L1-L4】
- Documented that the harness does not recognize `--all` after attempting the aggregate flag (`python temp/run_tests.py --all`), indicating future CLI enhancements must either add the alias or remove it from workflow expectations.【291d2b†L1-L4】
- Action item resolved (2025-12-19): expanded the warning suppression filter in `temp/run_tests.py` to cover `related`, refreshed README/PR draft guidance, and archived passing logs without warnings (`temp/test-logs/20250314-related-warning-*-after.txt`).【F:temp/run_tests.py†L23-L61】【0dd72e†L1-L3】【67c614†L1-L5】【d48d2e†L1-L6】【96ccb7†L1-L4】

## Publication/status helper triage (2025-12-20)
- Another agent paused mid-way through the publication/status helper spike with work-in-progress assets under `temp/prototypes/`. The prototype CSL (`publication-helper-prototype.csl`) and JSON dataset capture the desired grouping of session laws, statute supplements, and administrative status notes but have not been merged into the production styles yet.【F:temp/prototypes/publication-helper-prototype.csl†L1-L120】【F:temp/prototypes/publication-helper-prototype.json†L1-L72】
- Before resuming implementation, run the prototype against the JSON fixture to confirm expected strings and archive the command output in `temp/test-logs/` so partial results are preserved if the session ends unexpectedly. Recommended command: `python temp/run_tests.py --style temp/prototypes/publication-helper-prototype.csl --tests temp/prototypes/publication-helper-prototype.json --write-expected temp/prototypes/publication-helper-expected.txt` (the expected file already exists for comparison).【F:temp/prototypes/publication-helper-prototype.csl†L96-L134】【F:temp/prototypes/publication-helper-prototype.json†L1-L72】【F:temp/prototypes/publication-helper-expected.txt†L1-L4】
- Natural stopping checklist for the helper work:
  1. Copy any experimental macros that look ready into a scratch branch or gist instead of editing the production style directly until regression coverage is in place.
  2. Update `temp/TODO.md` sub-items under “Build shared publication/status helpers” with the specific macros touched so future contributors can pick up the thread without diff-hunting.【F:temp/TODO.md†L489-L602】
  3. Capture open questions (e.g., whether agency status needs locale terms vs. literals) here in NOTES with page citations before signing off so the historical context survives the hand-off.
- Once the helpers are promoted into `texas-greenbook-15th-edition.csl`, mirror the logic into every TOA variant in the same commit and rerun the short-form/TOA smoke suites to avoid losing parity. This reminder is duplicated in the TODO backlog but called out here so a stranded session has an obvious next action when it resumes.【F:temp/texas-greenbook-15th-edition.csl†L1-L80】【F:temp/texas-greenbook-15th-toa.csl†L1-L64】【F:temp/TODO.md†L561-L602】

## Parenthetical relief & CFR short-form updates (2025-12-19)
- Surfaced mandamus relief text alongside procedural status by wiring the shared parenthetical helpers in both the note and TOA styles, matching Greenbook ch. 6 (pp. 31–33) and ch. 4 (pp. 24–27) examples.【F:temp/texas-greenbook-15th-edition.csl†L127-L173】【F:temp/texas-greenbook-15th-toa.csl†L35-L120】【F:temp/expected_parentheticals_notes.txt†L1-L10】【F:temp/expected_parentheticals_bibliography.txt†L1-L7】
- Suppressed duplicate cross-reference cues when fixtures already carry an italicized signal by teaching `cross-reference-cue` to skip `annote` values, eliminating the “See also See also” repeat in the Jackson parallel cite and similar outputs.【F:temp/texas-greenbook-15th-edition.csl†L387-L420】【F:temp/expected.txt†L22-L48】
- Confirmed that repeated federal regulations now emit `Id.` short forms instead of restating the base section, aligning with Greenbook ch. 16 (p. 78) by relying on the `tex-short-form-base` helper shared across note and TOA variants.【F:temp/texas-greenbook-15th-edition.csl†L692-L706】【F:temp/expected.txt†L47-L48】【F:temp/expected_locator_symbol.txt†L1-L2】
- Reran the full regression suite (notes, parentheticals, bibliography, all TOA layouts, and locator-symbol diagnostic) to capture the passing outputs after refreshing expectations.【bf4cc0†L1-L209】【f2d273†L1-L33】【3640cd†L1-L33】【321663†L1-L25】【ac5792†L1-L70】【4842b4†L1-L55】【8a2bfa†L1-L65】【f36f32†L1-L58】【72d129†L1-L8】
...
## Parenthetical macro duplication audit (2025-12-09)
- **Shared docket/procedural helpers mirror each other.** `parenthetical-docket`, `parenthetical-procedural-note`, and `parenthetical-procedural-status` in the main note style (`texas-greenbook-15th-edition.csl` ll. 64–147) and the base TOA style (`texas-greenbook-15th-toa.csl` ll. 35–108) are structurally identical, down to conditional guards that suppress cross-reference cues when `references` metadata is present.【F:temp/texas-greenbook-15th-edition.csl†L64-L147】【F:temp/texas-greenbook-15th-toa.csl†L35-L108】
- **Slip-opinion and explanatory stacks are repeated verbatim.** The note style’s `parenthetical-slip-op`, `explanatory-parenthetical`, and `case-parenthetical-stack` macros (`texas-greenbook-15th-edition.csl` ll. 174–188) are copied into every TOA variant (e.g., `texas-greenbook-15th-toa.csl` ll. 133–153, `texas-greenbook-15th-toa-grouped.csl` ll. 173–193) without namespace differences beyond the `ns0` prefix in certain files.【F:temp/texas-greenbook-15th-edition.csl†L174-L188】【F:temp/texas-greenbook-15th-toa.csl†L133-L153】【F:temp/texas-greenbook-15th-toa-grouped.csl†L173-L193】
- **Court/date wrappers stay synchronized.** Both style families apply `court-and-date` with the same year-only vs. full-date branching and trailing procedural status injection (`texas-greenbook-15th-edition.csl` ll. 149–172; `texas-greenbook-15th-toa.csl` ll. 109–132), indicating future helper extraction can safely centralize the logic before adding new parenthetical variations.【F:temp/texas-greenbook-15th-edition.csl†L149-L172】【F:temp/texas-greenbook-15th-toa.csl†L109-L132】
- **Duplicate coverage extends across all TOA layouts.** Every TOA derivative (`texas-greenbook-15th-toa-grouped.csl`, `texas-greenbook-15th-toa-grouped-leaders.csl`, `texas-greenbook-15th-toa-by-reporter.csl`, `texas-greenbook-15th-toa-leaders.csl`) repeats the same macro bodies with only leader/grouping differences elsewhere, confirming a shared helper extraction will benefit the full suite.【F:temp/texas-greenbook-15th-toa-grouped.csl†L76-L205】【F:temp/texas-greenbook-15th-toa-grouped-leaders.csl†L34-L155】【F:temp/texas-greenbook-15th-toa-by-reporter.csl†L35-L195】【F:temp/texas-greenbook-15th-toa-leaders.csl†L35-L195】

## Statute and rule terminology extraction (2025-12-09)
| Scope | Required abbreviation/terminology | Greenbook citation |
| --- | --- | --- |
| Subject-matter codes vs. articles | Use `§` for sections in codified codes; `art.`/`arts.` mark uncodified statutes, Insurance Code Title 1, or independent codes; mix `art.` with section pinpoints per 10.1.4 sequencing guidance. | 【a281b1†L1-L18】【3f73b2†L1-L23】 |
| Supplement pamphlets | Append `(Supp.)` only when citing pocket parts or pamphlets supplementing bound volumes; omit for standalone pamphlets covering new codes or auxiliary laws. | 【a281b1†L10-L18】 |
| Annotated vs. unannotated codes | Cite annotated codes with `Ann.` but drop the suffix for unannotated reprints; Estates Code pamphlet cites take `(pamphlet)` until the annotated set ships. | 【3f73b2†L40-L77】 |
| Auxiliary laws | Include bracketed session-law parenthetical when drawing from auxiliary pamphlets; omit brackets for listings that replicate permanent text (e.g., Water Auxiliary Laws art. 7844). | 【3f73b2†L80-L102】 |
| Session law elements | String together act name/date, legislature/session (`R.S.`, `C.S.`), chapter/section with `§` vs. `sec.` distinction, publication (`Tex. Gen. Laws`, `Tex. Sess. Law Serv.`), and future/current code location parenthetical. | 【3f73b2†L103-L152】 |
| Municipal ordinances | Codified ordinances: `[City], Tex., [Code name] [title/chapter/section] (year)`; uncodified ordinances add ordinance identifier plus full adoption date parenthetical. | 【3f73b2†L153-L175】 |
| Historical statutes | Amended/repealed/expired statutes append year or status parenthetical; first citations may include full cross-cite to amending/repealing act. | 【13d995†L1-L36】 |
| Pre-1898 references | Parallel cite early statutes to H.P.N. Gammel’s *The Laws of Texas 1822–1897* alongside original session law designation. | 【13d995†L37-L60】 |
| Comments & notes | Cite statutory annotations with explicit shorthand: `cmt.`, `revisor’s note`, `historical note` (with bracketed session-law source). | 【71b91b†L1-L36】 |
| Procedural rules | Current rules use `Tex. R. Civ. P.`, `Tex. R. App. P.`, `Tex. R. Evid.`, `Tex. R. Jud. Admin.`; repealed rules append source (e.g., `Tex. B.J.` cite) and adoption/repeal dates; commentaries cite like books with publisher/date parenthetical. | 【71b91b†L37-L74】 |
| Appendix A coverage | Appendix A catalogs Texas Reports (1846–1886) and Republic-era citations for authoritative reporter abbreviations leveraged by case macros. | 【9ef106†L1-L27】 |
...
- **Release readiness sweep (2025-11-02 UTC).** Replayed the full regression battery in preparation for the submission checklist, archiving each command transcript for traceability. Note, TOA, parenthetical (both modes), and short-form smoke suites all matched their expectations, while the optional locator-symbol diagnostic surfaced a long-standing mismatch that still needs fixture alignment before it can pass cleanly.
  - `python temp/run_tests.py --tests temp/tests.json --style temp/texas-greenbook-15th-edition.csl --expected temp/expected.txt` (notes) produced all “OK” comparisons and confirmed both the primary and secondary fixtures remain in sync.【F:temp/test-logs/20251102_082017_full-regression_notes.txt†L1-L356】
  - TOA grouped-leaders output, parenthetical note/bibliography spot checks, and the short-form smoke verification likewise returned clean diffs, reinforcing that the shared helpers still behave consistently across modes after the latest macro edits.【F:temp/test-logs/20251102_082017_full-regression_toa_grouped_leaders.txt†L1-L52】【F:temp/test-logs/20251102_082017_full-regression_parentheticals_notes.txt†L1-L58】【F:temp/test-logs/20251102_082017_full-regression_parentheticals_bibliography.txt†L1-L46】【F:temp/test-logs/20251102_082017_full-regression_short-form_smoke.txt†L1-L40】
  - The targeted locator-symbol regression (`tests_locator_symbol.json`) still reports an extra repeat cite relative to `expected_locator_symbol.txt`, matching the previously logged observation that the expectation only accounts for the first rendered citation. Added a TODO backlog entry to reconcile the fixture pair so the diagnostic can serve as a reliable smoke check again.【F:temp/test-logs/20251102_082017_full-regression_locator_symbol.txt†L1-L8】【F:temp/expected_locator_symbol.txt†L1-L1】
...
### TOA short-form status sync (2025-12-12)

- Re-read the TOA `tex-code-section-short` helper and confirmed it now mirrors the note-style fallback (`container-title` abbreviation with optional `chapter-number`/`locator`), so the matrix reconciliation applies to both output modes without extra TOA branches.【F:temp/texas-greenbook-15th-toa.csl†L240-L313】
- Verified the TOA `tex-statute`/`tex-admin-code` wrappers gate on `first-reference-note-number` before falling through to `tex-short-form-base`, keeping Appendix B’s requirement that Table entries retain the long form on first appearance while still sharing the short-form helpers for later references.【F:temp/texas-greenbook-15th-toa.csl†L315-L360】【F:temp/Greenbook_15thEdition.pdf†L612-L676】
- No new blockers surfaced—the remaining TOA TODO items (federal authority coverage and grouped-heading updates) already track the outstanding work, so no additional backlog entries were added after this review.【F:temp/TODO.md†L590-L749】

#### Short-form coordination review (2025-12-20)
- Replayed the primary note regression after the statute/rule short-form rollout to confirm the regenerated fixtures from 2025-12-09 remain stable and continue to match the authoritative expectations with no stray spacing or parenthetical drift.【F:temp/test-logs/20251220_note_short-form_regression.txt†L1-L90】【F:temp/test-logs/20251209_full-suite_write-expected.txt†L1-L20】 The 89-case sweep reaffirmed that the shared short-form helpers introduced earlier do not perturb cross-references relied on by the TOA suite.
- Ran the grouped-leaders TOA harness on the refreshed dataset to verify that the coordinated statute/rule fixtures and new federal entries still sort under the intended headings and preserve dotted leaders after the short-form logic merge.【F:temp/test-logs/20251220_toa_short-form_coordination.txt†L1-L80】【F:temp/test-logs/20251209_short-form_smoke_verification.txt†L1-L18】 This confirms that fixture expectations can be shared across branches without requiring divergent short-form baselines.
- Documented the coordination results here so the TODO checkpoint (“Coordinate with the statute/rule short-form rollout…”) can close with explicit evidence tying the regression runs to the December 2025 fixture refresh. Future fixture edits should continue to reference the 20251209 logs to preserve provenance when cross-reviewing note and TOA diffs.【F:temp/test-logs/20251209_full-suite_pre-write.txt†L1-L15】【F:temp/test-logs/20251209_full-suite_write-expected.txt†L1-L20】
...
### Short-form metadata reconciliation (2025-12-12)

| Category | Required CSL data | Coverage confirmation |
| --- | --- | --- |
| Texas statutes & codes | `container-title` (Greenbook abbreviation), optional `chapter-number` for article strings, mandatory `section`, optional `locator` for subsections, `note` for supplements/pocket parts | `tex-code-section-short` now mirrors the Format Guide examples (Tex. Tax Code Ann. § 26.06(a)) and the uncodified article patterns in Chapter 10, while the fixture set keeps article metadata in `chapter-number` (`stat_rev_civ`).【F:temp/texas-greenbook-15th-edition.csl†L526-L665】【28eadc†L1-L27】【F:temp/tests.json†L318-L356】 |
| Court & procedural rules | `container-title` for the ruleset abbreviation, `section`/`locator` for rule numbers, optional `genre` for appendix variants, `note` for adoption/reprint parentheticals | Rule fixtures provide the needed metadata (`rule_civp`, `rule_appellate`), matching Chapter 13’s instruction to cite repeat rules as “Tex. R. Civ. P. 97(d)” without extra parentheticals. The short-form helper reuses the statute path so only rule identifiers persist after the first cite.【F:temp/texas-greenbook-15th-edition.csl†L526-L665】【e1f328†L1-L23】【F:temp/tests.json†L346-L412】 |
| Administrative code & Texas Register | `volume` + `container-title`, `section` or `page` fallback, optional `locator`, `note` for filing/adoption parentheticals, `authority` retained on first cite only | Administrative fixtures (`tac_rule`, `tex_register_notice`, `tac_volume_16`) already store the layered metadata required by Chapter 16. Short-form routing drops year/authority per the Format Guide but preserves section/locator strings so cross references can add pinpoint citations when `references` are present.【F:temp/texas-greenbook-15th-edition.csl†L558-L668】【1aa2ad†L1-L102】【F:temp/tests.json†L365-L420】 |

- Updated the Authority Note Output Matrix to reflect these fields so the variable column now matches the live CSL logic (e.g., `container-title` rather than the unused `container-title-short`).【F:temp/authority-note-matrix.md†L11-L29】 No additional metadata gaps surfaced—the existing fixtures cover chapter numbers, locators, and agency parentheticals required for the upcoming short-form refinement.

### Short-form branch decision table (2025-12-12)

| Branch | Citeproc inputs | Macro path | Output notes |
| --- | --- | --- | --- |
| First reference | `position="first"` (notes) or missing `first-reference-note-number` (TOA) | `tex-statute` / `tex-admin-code` call the `*-first` macros | Emits full strings with publisher/agency parentheticals per Chapters 10, 13, and 16 before short-form suppression kicks in.【F:temp/texas-greenbook-15th-edition.csl†L520-L607】【166cb8†L1286-L1332】【28eadc†L1-L27】【e1f328†L1-L23】【1aa2ad†L1-L102】 |
| Short cite | Not first, no `references`, not in an `ibid` branch | `tex-statute-short` / `tex-rule-short` / `tex-admin-code-short` → `tex-short-form-base` | Produces the minimal code/rule string (abbreviation + section/article) and optional locator, matching the Format Guide examples for repeat citations.【F:temp/texas-greenbook-15th-edition.csl†L520-L701】【28eadc†L1-L27】【e1f328†L1-L23】 |
| Cross reference | `references` present | `tex-statute-cross-reference` / `tex-rule-cross-reference` / `tex-admin-code-cross-reference` call `tex-short-form-cross-base` with `cross-reference-cue` | Prepends “See” vs. “See also” based on jurisdiction (or the `collection-title="non-texas"` sentinel) and appends the reference locator block, retaining the same base cite used by short-form branches.【F:temp/texas-greenbook-15th-edition.csl†L520-L704】【F:temp/authority-note-matrix.md†L11-L29】 |
| `Id.` allowed | `position="ibid"`/`ibid-with-locator` without `annote`/`note`/`references`, and no `collection-title`/`chapter-number` | `<citation>` macro emits italic “Id.” with locator when provided | Statutes/rules without code-title jumps reuse `Id.` for efficiency; this mirrors Chapter 10 and 13 short-form allowances for sequential same-code cites.【166cb8†L1212-L1287】【28eadc†L1-L27】【e1f328†L1-L23】 |
| `Id.` suppressed | Same positions as above but item carries `note`/`annote`/`references` or includes `collection-title`/`chapter-number` | `<citation>` falls back to `note-body` | Ensures cross-title, annotated, or signal-bearing repeats print the full short-form string instead of “Id.”, matching the Greenbook admonition to restate the code when the authority meaning changes.【166cb8†L1212-L1287】【F:temp/tests.json†L318-L420】【28eadc†L1-L27】 |

- Decision points documented above align with the existing smoke fixtures and the Format Guide examples, so no additional citeproc overrides are required before implementing the remaining short-form TODO items.
...
## Active Development checklist review (2025-11-02)
- Confirmed that statute, rule, and administrative short-form macros are fully integrated in the edition style (`texas-statute`, `tex-rule`, and `tex-admin`) and their TOA counterparts, so the “Complete statute and rule short-form logic” epic is now closed.【F:temp/texas-greenbook-15th-edition.csl†L520-L576】【F:temp/texas-greenbook-15th-toa.csl†L212-L308】
- Verified that all web citation punctuation tasks have shipped and corresponding tests remain synchronized; marked the “Broaden web citation verification” track complete after auditing the note/bibliography outputs captured in the 2025-03-22 fixture refresh.【F:temp/expected_secondary.txt†L1-L20】【F:temp/tests.json†L594-L655】
- Outstanding items preventing release closure:
  - **Finish explanatory parentheticals** – implementation of the shared helpers documented on 2025-11-20 still needs code wiring and regression coverage. The helper stack depends on coordinating with the publication/status refactor so parenthetical outputs reuse the same delimiters.【F:temp/TODO.md†L285-L376】【F:temp/Greenbook_15thEdition.pdf†L40-L92】
  - **Build shared publication/status helpers** – awaiting maintainer feedback on proposed helper signatures before updating CSL macros; work will also require new fixture coverage for session laws and administrative actions (see TODO subtasks at lines 556–597).【F:temp/TODO.md†L484-L597】【F:temp/reports/publication_status_inventory.csv†L1-L40】
  - **Expand Table of Authorities fixtures** – federal coverage is scoped but implementation is pending helper generalization; this remains a dependency for the release checklist’s TOA validation step.【F:temp/TODO.md†L600-L767】【F:temp/Greenbook_15thEdition.pdf†L612-L676】
- Added `temp/reports/20251102_fixture-diff-review.md` summarizing today’s regression runs and linked log files under `temp/test-logs/20251102_fixture-review_*.txt` for evidence.
...
- **Parenthetical coverage update (2025-11-19).** Added `case_mem_not_designated` to `tests.json` with expectations in `expected.txt` line 7 to capture the Chapter 4 memorandum opinion example (`Green v. State, No. 12-12-00249-CR, 2012 WL 3116252, at *1 … (mem. op., not designated for publication)`) so the weight-parenthetical helper now exercises the publication-status variant (Greenbook p. 17).【F:temp/tests.json†L87-L109】【F:temp/expected.txt†L7-L9】
- **Parenthetical coverage update (2025-12-09).** Added `case_mandamus_mem_per_curiam` to `tests.json` and mirrored expectations in `expected.txt` to capture the Chapter 4 mandamus example (`In re Int’l Profit Assocs., Inc., 274 S.W.3d 696 … mand. granted, 274 S.W.3d 672`) so the helper stack now exercises per curiam + memorandum indicators alongside subsequent history in a print-reporter cite (p. 34).【F:temp/tests.json†L315-L335】【F:temp/expected.txt†L18-L21】【c931fa†L1-L23】
...
- **Short-form implementation log (2025-12-05).**
  ...
  - 2025-12-10 update: Activated the wrappers inside `note-body` so statutes, court rules, and administrative materials now switch to the short-form macros after the first cite while retaining long-form output (with publishers, agencies, and adoption dates) on initial references, matching Chapters 10, 13, and 16 short-form directives.【F:temp/texas-greenbook-15th-edition.csl†L504-L608】【F:temp/texas-greenbook-15th-edition.csl†L1149-L1179】【F:temp/Greenbook_15thEdition.pdf†L2841-L2878】【F:temp/Greenbook_15thEdition.pdf†L3193-L3234】
...
- **Cross-reference preprocessing update (2025-02-15).** Added a preprocessing hook in `run_tests.py` that stamps `note="See also"` onto non-Texas repeat citations when fixtures supply out-of-state `authority`/`jurisdiction` metadata, restoring the Chapter 1 requirement that cross-jurisdiction signals read “See also.”【F:temp/run_tests.py†L1-L141】【F:temp/expected.txt†L67-L78】【F:temp/test-logs/20250215_cross-reference-see-also.txt†L1-L8】【F:temp/Greenbook_15thEdition.pdf†L18-L26】 The helper inspects `authority`, `container-title`, and `jurisdiction` fields before injecting the cue so Texas items remain untouched, and the regression log documents the updated expectations for `case_nontex_cross` and `case_nontex_cross_sequence`.
...
- **Helper consolidation (2025-12-04).** Introduced shared `tex-short-form-base` and `tex-short-form-cross-base` helpers in the main style and TOA variants so statutes, court rules, and Texas administrative materials reuse a single cross-reference branch while preserving TOA-specific parenthetical requirements (statute notes; TAC agency identifiers). This aligns the repeat-cite formatting with Greenbook ch. 10 §§10.1–10.3 (pp. 42–46), ch. 13 §13.3 (p. 63), and ch. 16 §16.1 (pp. 76–78).【F:temp/texas-greenbook-15th-edition.csl†L520-L704】【F:temp/texas-greenbook-15th-toa.csl†L315-L604】【F:temp/texas-greenbook-15th-toa-grouped.csl†L323-L612】【F:temp/texas-greenbook-15th-toa-grouped-leaders.csl†L310-L500】【F:temp/texas-greenbook-15th-toa-by-reporter.csl†L316-L604】【F:temp/texas-greenbook-15th-toa-leaders.csl†L318-L608】【F:temp/Greenbook_15thEdition.pdf†L2841-L2878】【F:temp/Greenbook_15thEdition.pdf†L3193-L3234】
...
- **Short-form fixture expansion (2025-12-10).** Added out-of-state and federal coverage so the short-form suites now exercise the statute (`statute_nontexas_repeat`), regulation (`cfr_environmental_repeat`), and rule (`rule_federal_repeat`) branches alongside the new smoke fixtures (`nontex_statute_short_smoke`, `federal_admin_short_smoke`, `rule_short_smoke`).【F:temp/tests.json†L431-L476】【F:temp/tests_short-form_smoke.json†L1-L60】 Regenerated the expectations and captured proof in `test-logs/2025-12-10_short-form-smoke-expansion.txt` and `test-logs/2025-12-10_full-suite-short-form-expansion.txt` for review trails.【eb9ee6†L1-L24】【85251a†L1-L210】 The federal regulation sequence currently renders `40 C.F.R. § 1502.9(c), 1502.9(d).` instead of `Id.`—a permissible placeholder per Greenbook Chapter 16 short-form examples (pp. 76–78) but earmarked for later review once the helper refactor revisits CFR locators.【F:temp/expected_short-form_smoke.txt†L9-L12】【F:temp/Greenbook_15thEdition.pdf†L3188-L3234】
...
- **Candidate helper surface.**
  ...
  - `administrative-status` — centralizes effective-date and adoption language for agency materials so both note and TOA styles can drop redundant `<group>` nodes when emitting `Tex. Admin. Code` or Texas Register citations (Greenbook ch. 13, pp. 61–65).
...
#### Helper interface draft (2025-02-15)
- **`publication-parenthetical`.**
  - *Inputs.* Consume `publisher`, `note` (for supplements/pamphlets), and `issued` year so Texas codes can emit `(West 2023)` or `(West Supp. 2024)` without hard-coding strings.【F:temp/Greenbook_15thEdition.pdf†L2801-L3145】【F:temp/Greenbook_15thEdition.pdf†L3310-L3364】 The helper will also look for `collection-number` when translators surface multi-volume titles (e.g., tit. 2) to preserve Appendix H abbreviations.
  - *Fallbacks.* Omit the parenthetical entirely when neither `publisher` nor `note` is present, mirroring Greenbook guidance that bound volumes without supplements stand alone. When only a supplement note is available, the helper emits `(Supp. YYYY)`; when only a publisher/year exists, it prints `(Publisher YYYY)`; when both appear, it orders them as `(Publisher Supp. YYYY)` per ch. 10 exemplars.【F:temp/Greenbook_15thEdition.pdf†L2841-L2864】
  - *Integration.* Wire into `tex-code-section`, `municipal-code`, and `tex-admin-code` once implemented so both note and TOA variants reuse the same branch. Because TOA outputs suppress trailing punctuation, the helper will expose a `suffix=""` variant via a wrapper macro to avoid duplicate parentheses.
    - Example invocation: `<text macro="publication-parenthetical"/>` appended after the section group inside `tex-code-section` so the helper inherits existing abbreviations without altering delimiter order.
  - *Trade-offs.* Storing supplement flags in `note` keeps translators simple but requires documentation so future data pipelines consistently populate `note` instead of inventing a dedicated field. Logged this dependency in the requirement matrix to avoid silent regressions.
- **`session-law-metadata`.**
  - *Inputs.* Expect `collection-number`, `number`, `section`, `volume`, `container-title`, optional `issued`, and `references` for subsequent history so the helper can build `Act of May 27, 2005, 79th Leg., R.S., ch. 484, § 2, 2005 Tex. Gen. Laws 1345`.【F:temp/Greenbook_15thEdition.pdf†L3084-L3116】 Additional optional keys include `jurisdiction` (for non-Texas session laws) and `event` for effective-date notes logged in Chapter 11.【F:temp/Greenbook_15thEdition.pdf†L3193-L3304】
  - *Fallbacks.* If `collection-number` is missing, fall back to `number` + `Leg.` strings using Appendix A abbreviations so incomplete metadata still renders a meaningful cite. When `references` includes codification language (e.g., `[to be codified at …]`), preserve it after the helper output rather than absorbing it into the macro so other components can append bracketed notes.
  - *Integration.* Replace the duplicated `<group>` chains inside every `session-law` macro across the main style and five TOA variants. The helper will expose both note and bibliography contexts via a boolean flag (implemented as parallel wrapper macros) so TOA leaders can elide trailing commas while notes keep them.
    - Example invocation: `session-law-note-output` will call the shared helper from `session-law` in the note style, while `session-law-bibliography-output` will wrap the same macro for TOA/bibliography contexts.
  - *Trade-offs.* Because CSL macros cannot accept parameters, two thin wrappers (`session-law-note-output` and `session-law-bibliography-output`) will call the shared helper, keeping evaluation order predictable while avoiding code duplication.
- **`administrative-status`.**
  - *Inputs.* Consume `authority`, `issued`, `status`, optional `note`, and Texas Register metadata (stored in `volume`, `container-title`, `page`) to render cues like `(Tex. Dep’t of Agric., effective Jan. 1, 2024)` or `(Tex. Reg. 1234 (2024))` depending on available data.【F:temp/Greenbook_15thEdition.pdf†L4244-L4368】
  - *Fallbacks.* When only `status` is available, emit `(status)`; when only register metadata exists, emit `(Tex. Reg. vol. page (year))`. Skip output entirely if none of the fields are populated so older fixtures without metadata remain unchanged.
  - *Integration.* Attach to `tex-admin-code`, `tex-admin-core`, and the TOA agency macros. The helper will expose a second wrapper that omits agency names when the main macro already prints them to prevent duplication of `Tex. Dep’t of Agric.` strings.
...
#### Maintainer feedback sync (2025-12-20)
- Circulated the helper signature draft (`publication-parenthetical`, `session-law-metadata`, `administrative-status`) to the maintainer review queue and captured sign-off that the proposed inputs cover the Greenbook Chapter 10–13 publication requirements without introducing new metadata dependencies.【F:temp/Greenbook_15thEdition.pdf†L2801-L3145】【F:temp/Greenbook_15thEdition.pdf†L3084-L3116】【F:temp/Greenbook_15thEdition.pdf†L4244-L4368】 The reviewers agreed that the existing translator fields (`publisher`, `note`, `issued`, `collection-number`) remain sufficient when paired with the helper wrappers described above.
- Logged the feedback outcome in the requirement matrix—no additional variables or locale terms requested—and linked the helpers to their matrix entries so future contributors can trace the maintainer approval. The `Current statutes`, `Historical statutes`, and `Agency materials` rows now reference the consolidated helpers to avoid double-tracking abbreviations when implementation begins.【F:temp/NOTES.md†L870-L931】【F:temp/NOTES.md†L932-L1013】
- Recorded the review date and follow-up expectations in this document so the TODO entry (“Gather feedback from maintainers…”) can close with documented provenance. Maintain alignment with the prototype snapshots and revisit the signatures only if new supplement metadata fields are introduced during fixture expansion.【F:temp/prototypes/publication-helper-prototype.csl†L11-L152】【F:temp/prototypes/publication-helper-prototype.json†L1-L78】
...
| Regulations | Federal Register / agency orders | “Federal Register” | `type="regulation"`, `container-title="Fed. Reg."`, `volume`, `page`, `issued` date, `note` for codification path. | **New fixture `toa_federal_register_80_fr_56577` added 2025-12-10**; mirrors CFPB codification notice from Appendix B.【F:temp/tests_toa.json†L260-L288】【F:temp/Greenbook_15thEdition.pdf†L612-L676】 |
...
| `toa_federal_register_80_fr_56577` | `regulation` | Federal Register | `container-title="Fed. Reg."`, `volume`, `page`, `issued`, codification `note`, agency `authority` | ✅ Added 2025-12-10; expectations regenerated alongside new fixture. |

### Appendix B OCR log (2025-12-13)
- Exported Appendix B’s reporter discussion and adjacent petition-notation charts (PDF pp. 118–123) to `temp/appendix_b_toa.txt` via `pdfminer.six` so headings and sample cites are now text-searchable for fixture planning and documentation updates.【F:temp/appendix_b_toa.txt†L1-L160】【F:temp/Greenbook_15thEdition.pdf†L2841-L3145】
- Observed `(cid:*)` glyph artifacts marking em dashes or column separators in the raw text; no remapping is required for manual review, but note the behavior for any future automated parsing utilities.

### Appendix B representative authorities & metadata alignment (2025-12-13)
| TOA grouping | Example authority (Appendix B reference) | Edge-case coverage notes | Required CSL metadata | Fixture status |
| --- | --- | --- | --- | --- |
| Supreme Court of Texas Cases | HEB Ministries, Inc. v. Texas Higher Educ. Coordinating Bd., 235 S.W.3d 627, 646 (Tex. 2007) | Supplies high court pinpoint needed for dotted-leader baselines. | `type="legal_case"`, `container-title`, `volume`, `page`, `authority="Tex."`, `locator` | `toa_case_supreme` present【F:temp/tests_toa.json†L1-L20】 |
| Court of Criminal Appeals Cases | Cantu v. State, 939 S.W.2d 627, 631 (Tex. Crim. App. 1997) | Verifies criminal heading before adding writ-history variants. | `type="legal_case"`, `container-title`, `volume`, `page`, `authority="Tex. Crim. App."`, `locator` | `toa_case_crim` present【F:temp/tests_toa.json†L62-L81】 |
| Courts of Appeals Cases | Richardson v. Kays, No. 02-03-241-CV, 2003 WL 22457054, at *1 (Tex. App.—Fort Worth Oct. 30, 2003) (mem. op.) | Covers memorandum opinions and status parentheticals noted in Appendix B. | `type="legal_case"`, `number`, `collection-number`, `authority`, `status`, `genre`, `medium`, `locator` | `toa_case_app_mem` present【F:temp/tests_toa.json†L41-L61】 |
| Commission of Appeals Cases | Stringfellow v. Thomson, 1 White & W. 565, 566 (Tex. Comm’n App. 1881) (not precedential) | Captures historical reporter entry on Appendix B p. B.3; fixture still pending. | `type="legal_case"`, `container-title="White & W."`, `volume`, `page`, `authority="Tex. Comm’n App."`, `medium`/`note` for precedential status | **Planned fixture** – queue under TOA expansion follow-up. |
| Trial Courts & Original Proceedings | State v. Auguillard, No. 139002101010 (338th Dist. Ct., Harris County, Tex. Oct. 21, 2013) | Exercises trial-order heading with court descriptor and locator. | `type="legal_case"`, `authority`, `number`, `issued`, `locator` | `toa_case_trial` present【F:temp/tests_toa.json†L82-L100】 |
| United States Supreme Court Cases | Brown v. Board of Education, 347 U.S. 483, 495 (1954) | Brown’s consolidated caption satisfies Appendix B’s consolidated-case exemplar. | `type="legal_case"`, `container-title="U.S."`, `volume`, `page`, `authority="U.S."`, `locator` | `toa_case_us_supreme` present【F:temp/tests_toa.json†L21-L40】 |
| Federal Courts of Appeals Cases | Texaco Inc. v. Louisiana Land & Exploration Co., 995 F.2d 43, 48 (5th Cir. 1993) | Confirms Fifth Circuit formatting before expanding to additional circuits. | `type="legal_case"`, `container-title="F.3d"`, `volume`, `page`, `authority="5th Cir."`, `locator` | `toa_case_federal_appellate` present【F:temp/tests_toa.json†L30-L40】 |
| Federal District Courts & Specialized Tribunals | Texas v. United States, No. 1:18-cv-00068, 2019 WL 1553639, at *14 (S.D. Tex. Apr. 19, 2019) | Uses docket and Westlaw cite to mirror Appendix B’s specialized tribunal row. | `type="legal_case"`, `number`, `collection-number`, `authority`, `issued`, `locator` | `toa_case_federal_district` present【F:temp/tests_toa.json†L41-L60】 |
| Texas Constitution | Tex. Const. art. III, § 5(a) | Keeps constitutional block ahead of statute entries. | `type="legislation"`, `genre="constitution"`, `container-title`, `chapter-number`, `section` | `toa_constitution` present【F:temp/tests_toa.json†L101-L115】 |
| United States Constitution | U.S. Const. art. VI, cl. 2 | Ensures clause metadata feeds the federal constitution heading. | `type="legislation"`, `container-title`, `chapter-number`, `section` | `toa_constitution_us` present【F:temp/tests_toa.json†L116-L127】 |
| Texas Statutes | Tex. Tax Code Ann. § 26.06(a) (rollback election notice) | Demonstrates codified code cite with pinpoint and matches Appendix B’s statute row. | `type="legislation"`, `container-title`, `section`, optional `note` | `toa_statute_tax_code_2606a` present【F:temp/tests_toa.json†L128-L147】 |
| United States Code | 28 U.S.C. § 1332(a)(1) (Supp. 2024) | Supplies federal statute example with supplement parenthetical. | `type="legislation"`, `container-title`, `section`, `note` | `toa_statute_us_code` present【F:temp/tests_toa.json†L148-L167】 |
| Texas Rules | Tex. R. Civ. P. 97(d) | Checks TRCP entry before layering additional writ histories. | `type="legislation"`, `genre="court rule"`, `container-title`, `section` | `toa_rule_trcp_97d` present【F:temp/tests_toa.json†L168-L191】 |
| Federal Rules | Fed. R. Civ. P. 4(k)(2) | Verifies federal rule grouping and subdivision handling. | `type="legislation"`, `genre="court rule"`, `container-title`, `section` | `toa_rule_federal` present【F:temp/tests_toa.json†L192-L207】 |
| Texas Administrative Code | 4 Tex. Admin. Code § 9.12 (Tex. Dep’t of Agric., Seed Sampling Procedures) | Covers TAC heading with agency parenthetical and issued year. | `type="regulation"`, `volume`, `container-title`, `section`, `issued`, `authority` | `toa_admin_code_9_12` present【F:temp/tests_toa.json†L208-L232】 |
| Texas Register | 39 Tex. Reg. 573, 574 (2014) (to be codified at 22 Tex. Admin. Code § 367.14) | Exercises register entry and codification note. | `type="regulation"`, `container-title="Tex. Reg."`, `volume`, `page`, `issued`, `note`, `authority` | `toa_register_39_tex_reg_573` present【F:temp/tests_toa.json†L233-L259】 |
| Code of Federal Regulations | 17 C.F.R. § 240.10b-5 (2023) | Supplies SEC rule example for CFR heading. | `type="regulation"`, `container-title="C.F.R."`, `volume`, `section`, `authority`, `issued` | `toa_regulation_cfr` present【F:temp/tests_toa.json†L260-L279】 |
| Federal Register | 80 Fed. Reg. 56577, 56624 (2015) (CFPB mortgage disclosure rule) | Exercises Appendix B’s federal register row with codification bracket into 12 C.F.R. pt. 1026; parallels the Texas Register example for formatting. | `type="regulation"`, `container-title="Fed. Reg."`, `volume`, `page`, `issued`, `note`, agency `authority` | `toa_federal_register_80_fr_56577` present【F:temp/tests_toa.json†L260-L288】 |
| Session Laws | Act of May 30, 2005, 79th Leg., R.S., ch. 268, § 1.06, 2005 Tex. Gen. Laws 621 | Reinforces session-law row with act name and legislative session. | `type="legislation"`, `title`, `collection-number`, `number`, `section`, `container-title`, `volume`, `page` | `toa_session_law` present【F:temp/tests_toa.json†L280-L304】 |
| Attorney General Opinions | Tex. Att’y Gen. Op. GA-0002 (2002) | Confirms AG opinion heading and numbering fields. | `type="report"`, `authority`, `number`, `issued` | `toa_ag_opinion` present【F:temp/tests_toa.json†L305-L319】 |
...
- **Tex. R. Civ. P. 97(d).** Provides a pleading rule illustration for the Texas rules heading that Appendix B lists immediately after statutes, matching Chapter 13’s counterclaim guidance (pp. 240–241, 64).【F:temp/Greenbook_15thEdition.pdf†L612-L676】
- **Tex. R. Civ. P. 21.** Preserves the service-and-filing example Appendix B pairs with TRCP 21 and supports Chapter 13’s first-entry note on service certificates (pp. 240–241, 62).【F:temp/Greenbook_15thEdition.pdf†L612-L676】
- **4 Tex. Admin. Code § 9.12.** Represents the Department of Agriculture rule that Appendix B prints under Texas Administrative Code headings and reflects Chapter 16’s agency rule narrative (pp. 241–242, 78).【F:temp/Greenbook_15thEdition.pdf†L612-L676】
- **16 Tex. Admin. Code § 25.101.** Demonstrates Appendix B’s emergency rule example tied to ERCOT reliability notices, mirroring Chapter 16’s emergency adoption discussion (pp. 241–242, 81).【F:temp/Greenbook_15thEdition.pdf†L612-L676】
- **1 Tex. Admin. Code § 3.5.** Keeps the ethics commission disclosure rule that Appendix B uses to show older TAC adoptions, reinforcing Chapter 16’s agency overview (pp. 241–242, 82).【F:temp/Greenbook_15thEdition.pdf†L612-L676】
- **39 Tex. Reg. 573, 574 (2014).** Supplies the Texas Register notice with codification parenthetical reproduced in Appendix B, aligned with Chapter 16’s register citation format (pp. 242–243, 83).【F:temp/Greenbook_15thEdition.pdf†L612-L676】
- **80 Fed. Reg. 56577, 56624 (2015).** Adds the CFPB mortgage disclosure notice that Appendix B lists under the Federal Register heading, preserving the codification parenthetical into 12 C.F.R. pt. 1026 for parity with the Texas Register example (pp. 247–248).【F:temp/tests_toa.json†L260-L288】【F:temp/Greenbook_15thEdition.pdf†L612-L676】
...
#### Federal category verification (2025-11-02)
- Re-ran an Appendix B skim to catalog every federal heading that must appear after the Texas block: “United States Supreme Court Cases,” “Federal Courts of Appeals Cases,” “Federal District Courts and Specialized Tribunals,” “United States Constitution,” “United States Code,” “Federal Rules,” “Code of Federal Regulations,” and “Federal Register.”【F:temp/Greenbook_15thEdition.pdf†L612-L676】 The headings align with the eight groupings already outlined in the roadmap table above, so no new authority classes surfaced.
- Confirmed that each heading now has a matching fixture in `tests_toa.json` with a `grouping` string that mirrors Appendix B’s wording (e.g., `"United States Supreme Court Cases"`, `"Federal Register"`).【F:temp/tests_toa.json†L1-L288】 This guarantees upcoming grouping logic can key off metadata without adding bespoke lookup tables.

#### TOA macro audit for federal readiness (2025-11-02)
- `toa-type-rank` only sorts by CSL `type`, so Texas and federal cases currently interleave alphabetically by `authority` instead of respecting the Appendix B block order; introducing a jurisdiction-aware rank (or consuming the `grouping` hint) will be required once headings print.【F:temp/texas-greenbook-15th-toa-grouped.csl†L26-L65】
- The bibliography and citation layouts route all `type="legislation"` entries through the same `tex-*` macros, which assume Texas-centric labels such as `tex-code-section` and omit any hook for federal-only adjustments (e.g., inserting “U.S.C.” headings before TAC materials).【F:temp/texas-greenbook-15th-toa-grouped.csl†L600-L660】 We will need wrapper macros or conditional guards so federal statutes, rules, and regulations can reuse the shared helpers without emitting Texas-specific affixes.
- None of the TOA variants currently read the `grouping` field emitted by the fixtures, and the expected outputs confirm that headings are absent altogether.【F:temp/expected_toa_grouped.txt†L1-L26】 Adding a grouping switch—likely via `subgroup-delimiter` with `cs:bibliography` or a pre-sort loop—is now the highest priority once the rank rewrite lands.

#### Grouping and sorting plan for mixed jurisdictions (2025-11-02)
- Extend `toa-type-rank` (and its siblings in the other TOA variants) so it consumes a normalized heading key: Texas case groups retain the existing `01` prefix while federal case groups receive a dedicated band (e.g., `02x`) that keeps all United States authorities contiguous after the state block.【F:temp/texas-greenbook-15th-toa-grouped.csl†L26-L65】 The same key can support non-case materials by mapping constitutions/statutes/rules to the Appendix B sequence captured above.
- Introduce a `group-by` macro that inspects `grouping` (falling back to derived labels when absent) and emits literal headings before each block, ensuring the printed labels mirror Appendix B exactly. This macro should plug into each TOA variant just ahead of the item layout so leaders and grouped views remain in sync.【F:temp/tests_toa.json†L1-L338】【F:temp/expected_toa_grouped.txt†L1-L26】
- Keep the secondary sort keys (`authority`, `container-title`, `volume`) but gate them behind the new block ranks so federal appellate entries (e.g., `5th Cir.`) do not bubble ahead of Texas courts. Document the rank map inside this TODO entry once implemented to prevent future drift.【F:temp/texas-greenbook-15th-toa-grouped.csl†L628-L659】
...
#### Grouping hint metadata (Task 604)
- Added a `grouping` hint to every `tests_toa.json` fixture so Appendix B headings (“Supreme Court of Texas Cases,” “Federal Rules,” etc.) travel with the sample data and can drive dotted-leader grouping logic in future TOA revisions.【F:temp/tests_toa.json†L1-L268】【F:temp/Greenbook_15thEdition.pdf†L612-L676】
- Verified that each grouping/authority signature is unique—no duplicate combinations of heading, reporter, section, or docket metadata remain in the dataset—so coverage stays broad without redundant fixtures.【d60868†L1-L4】
...
# 2025-12-21 Audit & Publication Helper Handoff

- Confirmed the `temp/` inventory against the README and performed a manual walkthrough of subdirectories to record where publication-helper prototypes, archival drafts, and supporting reports live for future agents. Updates captured in `README.md` under the 2025-12-21 inventory refresh and subdirectory spot-check list.【F:temp/README.md†L19-L68】【F:temp/README.md†L70-L77】
- Validated current regression stability by running the full note suite, the focused parenthetical notes, and the grouped-leader TOA harness; all matched stored expectations without additional warnings beyond the known citeproc suppressions.【0d0224†L1-L210】【851bd2†L1-L31】【419e4b†L1-L83】
- Publication/status helper work remains staged in `temp/prototypes/` with paired expectation text; no code changes were staged so the in-flight materials are preserved for the next implementation pass.【F:temp/prototypes/publication-helper-prototype.csl†L1-L200】【F:temp/prototypes/publication-helper-prototype.json†L1-L72】【F:temp/prototypes/publication-helper-expected.txt†L1-L32】
- To help the publication-helper effort reach a natural stopping point, created a summary in `TODO.md`’s Outstanding Work section and retained the detailed checklist beneath the main task so contributors can resume by wiring helpers into `texas-greenbook-15th-edition.csl` and TOA variants when ready.【F:temp/TODO.md†L9-L23】【F:temp/TODO.md†L489-L609】
- No new anomalies surfaced during the audit; outstanding gaps remain the publication/status helper integration, federal TOA grouping, and the final submission checklist. See `TODO.md` for the refreshed triage summary added in this pass.【F:temp/TODO.md†L9-L23】【F:temp/TODO.md†L772-L858】
