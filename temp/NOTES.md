...
- Confirmed the primary note regression continues to pass all 89 fixtures via `python temp/run_tests.py --tests temp/tests.json --style temp/texas-greenbook-15th-edition.csl --expected temp/expected.txt`; citeproc still emits a `UserWarning` about unsupported `related` metadata at the start of the run.【ff6d67†L1-L188】
- Exercised the focused parenthetical suites in both note and bibliography modes to verify slip-opinion URL placement and petition-history strings remain stable (`python temp/run_tests.py --tests temp/tests_parentheticals.json --style temp/texas-greenbook-15th-edition.csl --expected temp/expected_parentheticals_notes.txt` and the bibliography variant with `--mode bibliography`), both of which repeat the `related` warning despite matching expectations.【3cd12f†L1-L36】【2fb238†L1-L28】
- Ran the grouped-leaders TOA harness (`python temp/run_tests.py --tests temp/tests_toa.json --style temp/texas-greenbook-15th-toa-grouped-leaders.csl --expected temp/expected_toa_grouped_leaders.txt`) to confirm dotted leader alignment and restored authority inventory, observing the same unsuppressed warning at invocation.【8c3d03†L1-L52】
...
- Action item: extend the warning suppression filter in `temp/run_tests.py` to include `related` and refresh README guidance once the logs are quiet again.【F:temp/run_tests.py†L25-L34】
